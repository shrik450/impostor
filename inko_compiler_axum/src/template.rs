...
impl StringOrTemplate {
    ...
    pub(crate) fn from_ast_template(template: Template) -> StringOrTemplate {
        if template
            .elements
            .iter()
            .any(|element| matches!(element, inko_core::ast::TemplateElement::Expression(_)))
        {
...
pub fn execute(
    ...
    for element in template.elements.iter() {
        match element {
            inko_core::ast::TemplateElement::String { encoded, .. } => result.push_str(encoded),
            inko_core::ast::TemplateElement::Expression(expression) => {
