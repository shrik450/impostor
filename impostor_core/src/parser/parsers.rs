...
pub fn impostor_file(reader: &mut Reader) -> ParseResult<ImpostorFile> {
...
mod tests {
    ...
    fn test_basic_get_mock() {
        ...
        let impostor_file = impostor_file(&mut reader).unwrap();
        assert_eq!(impostor_file.entries.len(), 1);
        assert_eq!(reader.state.cursor, mock.len());
        println!("{:#?}", impostor_file);
    }
}
